...
\title{Disjoint Amalgamation Property (dAP)}
...
\begin{document}
\maketitle
\par{Given a class and relation \((K, \leq )\), we say that \(K\) has the \emph{Disjoint Amalgamation Property} if for every \(A,B,C  \in  K\) such that \(B \cap  C =A\), if there 
